...
- `thread/tokenUsage/updated` — `{ threadId, turnId, tokenUsage }` where `tokenUsage` is `{ total, last, modelContextWindow, contextBaseline }`. `contextBaseline` is `{ systemPromptTokens, toolSchemaTokens, outputSchemaTokens, toolCount }` measured when the request this event reports usage for was built. It is `null` whenever no such measurement belongs to that request — before the first one, for requests Codex builds without tools or an output schema (compaction), and while `last` is a locally recomputed estimate rather than a provider figure — which means denominate against Codex's own historical constant, not that the previous value still applies. The token figures are approximate — they use the same byte-density heuristic Codex applies to history estimation, not a tokenizer — so treat them as a denominator, never as a substitute for the provider-reported totals in `total` and `last`. Their sum is what a client should subtract from `modelContextWindow` to show how much of the window a conversation can still grow into; `toolCount` is exact.
